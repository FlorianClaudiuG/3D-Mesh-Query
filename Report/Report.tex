...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.7\linewidth]{Pictures/triangle.png}
...
\subsubsection{PCA}
Prinicipal component analysis is done using the arglib library. The three eigenvectors $e_1$, $e_2$, and $e_3$ are returned by the algorithm. These three vectors are orthogonal to each other. Therefore these eigenvectors can easily be made to coincide with the x, y, and z axis, because the model only has to be rotated. A rotation matrix M is than created using the vectors $x$, $y$, and $z$, that represent the axises of the normal coordinate system. The eigenvector corresponding to the largest eigenvalue will coincide with the x asis, the next eigenvector with the y axis, and the final one with the z axis.
...
Some characteristics of a staircase are present in these pictures, namely the large number of 90 degree edges leading to the top. S feature could be added that would encapsulate this characteristic so that all these models will have a distance of 0 for this feature. However even in this case all the other feature value will be very different, making it very unlikely that their overall distances will be small enough to yield good results.
